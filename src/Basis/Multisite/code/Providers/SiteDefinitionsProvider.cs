...
namespace Sitecore.Basis.Multisite.Providers
{
    public class SiteDefinitionsProvider : ISiteDefinitionsProvider
    {
        ...
        public IEnumerable<SiteDefinition> SiteDefinitions
        {
            get
            {
                if (this.siteDefinitions != null)
                    return this.siteDefinitions;

                this.siteDefinitions = this.sites.Where(this.IsValidSite).Select(this.Create).OrderBy(s => s.Item.Appearance.Sortorder).ToArray();
                return this.siteDefinitions;
            }
        }
        ...
        private static bool IsSite([NotNull] Item item)
        {
            ...
            return item.IsDerived(Templates.__Site.TemplateID);
        }
